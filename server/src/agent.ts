...
interface ToolContext {
  ...
  signal: AbortSignal
  toolCallId?: string
  // sub-agent orchestration: provider/model for delegate_task spawned LLM
...
export async function executeTool(name: string, argsJson: string, ctx: ToolContext): Promise<ToolExecResult> {
  ...
  switch (name) {
    ...
    case 'delegate_task': {
      ...
      // Record initial chat for sub-agent (user task) + mark working for live UI
      ...
      // Async execution: filtered tools per mode, real LLM if provider available else mock
      void (async () => {
        const subId = sa.id
        ...
        // Resolve provider/model: prefer ctx override, else modelId requested, else first available
        ...
        // If no provider/model, mock completion after short delay
        ...
        // Real LLM execution with filtered tools per mode
        const modeTools = (() => {
          ...
          const allNames = new Set(AGENT_TOOLS.map(t => t.function.name).concat(['delegate_task']))
        ...
        // Build history for sub-agent: system + task
        ...
        // need to allow sub-agent to emit its own tool events but also persist as sub-agent messages
        let acc = ''
        try {
          ...
          // Execute tool calls if any (sub-agent mini-loop: one round for MVP, full loop if needed)
          ...
          for (const call of result.toolCalls) {
            try {
              ...
              // also emit tool events for UI live activity
