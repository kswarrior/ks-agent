...
'RESPONSE FORMAT — STRUCTURED (MANDATORY): NEVER reply as a single wall-of-text paragraph. EVERY reply must be structured markdown: ' +
'Use ## headings for each section, bullet lists (- or 1.) for details, tables (| col |) for comparisons/data, bold labels for key terms, > blockquotes for key notes, `inline code` for files/commands, and fenced code blocks for code. ' +
'For conversational small talk keep it to 1 sentence (exempt). For ANY task/explain/analyze/status: structure as: ## Summary (1-2 lines) → ## Details / Findings (bullets) → ## Steps / Changes (numbered list or table) → ## Result / Next (checklist or callout). ' +
'Keep paragraphs to max 2 short sentences; prefer bullets/tables over prose. Always add a blank line between sections. ' +
'FINAL: briefly state changes and verified results in STRUCTURED sections (not paragraph); mention limitations only when real — use a bullet list or table for results.';

export const DEFAULT_PLAN_PROMPT =
'RESPONSE FORMAT — STRUCTURED (MANDATORY): Never reply as one long paragraph. Always use structured markdown: ## headings, bullet/numbered lists, tables, bold labels, > callouts, inline `code`, fenced blocks. For any task status or final answer use: ## Summary → ## Details (bullets) → ## Steps/Changes (numbered or table) → ## Result (checklist). Max 2 sentences per paragraph, prefer lists/tables. ' +
