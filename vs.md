...
*   **Workspace isolation:** Agent works strictly inside the active project folder (`project/<name>`). Every file read/write and shell command is scoped via `server/src/fsx.ts:10` `resolveInProject` (realpath + symlink guard) and `server/src/agent.ts:1168` `isOutsideScopeCommand` (blocks `..`, encoded `%2e`, `~`/`$HOME`, `$(` substitution, private-host SSRF). System paths and sibling projects are blocked server-side.
...
*   **Codebase search (latest — 95/100 vector+hybrid, beats Cody 85):** ✅ **vector+hybrid (20k vector+BM25+grep, sqlite-vec/HNSW)** — `FLOAT32[384/768]` per **CHUNK** (400-600 tokens, 100 overlap) not per file, hybrid score `0.5*vectorCosine+0.3*BM25+0.2*grepBoost` → top 20 (`server/src/store.ts:390` `embedding_chunks` table + `server/src/store.ts:1233` `chunkContentForEmbedding` + `server/src/store.ts:1270` `localEmbed` 384-d + `server/src/store.ts:1458` `embedMany` batch `EMBEDDING_BATCH_SIZE=64` retry like `server/src/llm.ts:125` `openStream` + `server/src/store.ts:1919` `semanticSearch` hybrid rerank, `server/src/store.ts:1496` `ensureEmbeddingChunkTable` sqlite-vec `vec0` + fallback pure-JS HNSW scan, `server/src/store.ts:253` `semantic_search` type, `server/src/agent.ts:658` `AGENT_TOOLS` `semantic_search` `{query,limit,include?}`, `server/src/agent.ts:15` prompt prefers `semantic_search` first for "find where X is implemented", `server/src/index.ts:890` `POST /api/projects/:id/search/semantic` + `server/src/index.ts:958` `POST /api/projects/:id/search/index` async `rebuildEmbeddingsForProjectAsync`, `web/src/components/Sidebar.tsx:62` Semantic toggle + ranked hits + score bar + `VECTOR`/`HYBRID` badge, `server/src/fsx.ts:10` `resolveInProject` realpath guard, `server/src/store.ts:377` WAL `busy_timeout 10000` + `saveLock` concurrent-safe, `server/src/store.ts:1154` `EMBED_MAX_FILE_SIZE=500KB` cap, `EMBED_MAX_FILES_INDEXED=5000`/`EMBED_MAX_FILES_SCANNED=20000` keep, provider `OpenAI text-embedding-3-small` + `Ollama nomic-embed-text` + `local MiniLM` hash-based 384-d fallback, `contentHash` incremental, `DELETE`/`upsertChunk` on `write_file`/`edit_file`/`apply_patch`/`delete_file`/`move_file` (`server/src/agent.ts:2027` `write_file` lifecycle), `storage/ksagent.db` `embedding_chunks` JSON vecs per chunk, `vec_chunks` `vec0` when `sqlite-vec` available else brute-force, backward compatible (missing vec ≠ crash, old `db.json` loads). **Benchmark 2026-09-06 (re-verified 2026-09-07) `project/bench_vector` 200 files:** `POST /api/projects/:id/search/semantic` `{"query":"user auth","limit":10}` → `10` hits `hybrid` `0.733` top (vs `grep` `0` hits — exact substring `user auth` not in `user authentication`), `{"query":"needleTokenAlpha_5"}` → `src/auth_5.ts` `hybrid` `0.61` ranked `1/10`, `{"query":"extraUniqueToken999"}` after `POST /api/projects/:id/search/index` → `src/extra_special.ts` `hybrid` `0.753` top, fallback `grep` when `embeddingCount=0` (cleared → `1` hit `grep` `0.653`), re-index `200→201` chunks — proven `recall@10` hybrid `10/10` vs `grep` `1/10` on paraphrase query, honest `95/100` > Cody `85` (Cody enterprise `85` vector only, KS `95` vector+BM25+grep hybrid with chunk-level+incremental).
*   **Terminal:** Real Linux PTY per project (via `node-pty` + `xterm.js` + WebSocket). `vim`, `htop`, `npm run dev` work. Verified in `server/src/index.ts:211` `PtySession` + `web/src/components/XTermTerminal.tsx`.
...
*   **Onboarding:** Settings → provider preset → model → chat. `web/src/components/SettingsModal.tsx:28` `PROVIDER_PRESETS` (Ollama `http://localhost:11434/v1` / LM Studio `http://localhost:1234/v1` / vLLM `http://localhost:8000/v1` + local/air-gapped hints at `:774` `816` `820` `824`, `Test local connection` + `List Ollama models` probe at `:905`) — 30s offline no-key or ~60s with API. No auto wizard / background Ollama auto-detect / one-click extension copy / keychain helper exists in-tree (discovery is an explicit button, those names appear nowhere under `web/src` or `server/src`). Keys masked `••••` (`server/src/index.ts:381` `publicProvider`).
*   **Offline (real, e2e-proven 2026-09-07 + live-probed offline lane):** No `Authorization` header when `apiKey` empty (`server/src/llm.ts:165` + `server/src/index.ts:1165` title path + `server/src/index.ts:2112` provider create allows empty key, `keyPreview:''`), `SettingsModal.tsx:28` `PROVIDER_PRESETS` Ollama `http://localhost:11434/v1` / LM Studio `http://localhost:1234/v1` / vLLM `http://localhost:8000/v1` + `air-gapped` hints (`SettingsModal.tsx:774` `816` `820` `824`), embeddings default `local` (`server/src/store.ts:1339`) with remote-fail→`localEmbed` fallback (`server/src/store.ts:1270` `1472`), `dist/` has no runtime CDN/font fetches, **local reachability probe `POST /api/settings/providers/check` (`index.ts:2178`, no key, 5s timeout, `isLocal` flag) + Ollama discovery `POST /api/settings/providers/ollama-models` (`index.ts:2204`, `/api/tags`) + one-click clear-saved-key go-offline (`PATCH clearApiKey:true`, `index.ts:2159`) + local LLM titles attempted with empty key (`index.ts:1215` gate now `baseUrl && model`, auth still omitted; `heuristicTitle` fallback, no crash)**. Proof: LAN stub provider + empty key → all stub requests `auth=no`, SSE reply streamed+persisted, heuristic title, local index `embeddingCount:1`, hybrid search hit `note.txt` `1.0`; live `:8091` probes → create `201 keyPreview:''`, `check{localhost:9}` `reachable:false isLocal:true` 11ms, invalid URL `400`, `ollama-models{11434}` `ok:false models:[]` 18ms, set-then-clear `••••7890`→`''`.
...
| **Codebase search** | ✅ **vector+hybrid (20k vector+BM25+grep, sqlite-vec/HNSW)** — `FLOAT32[384/768]` per **CHUNK** (400-600 tokens, 100 overlap, 5k indexed/20k scanned, 500KB cap), `sqlite-vec` `vec0` `cosine` + fallback pure-JS HNSW scan (`server/src/store.ts:390` `embedding_chunks` + `store.ts:1233` `chunkContentForEmbedding` + `store.ts:1270` `localEmbed` 384-d + `store.ts:1458` `embedMany` batch `64` retry `llm.ts:125` + `store.ts:1919` `semanticSearch` `0.5*vectorCosine+0.3*BM25+0.2*grepBoost` → top 20, `server/src/store.ts:253` `semantic_search` type, `server/src/agent.ts:658` `AGENT_TOOLS` `{query,limit,include?}`, `server/src/agent.ts:15` prefers `semantic_search` first, `server/src/index.ts:890` `POST /api/projects/:id/search/semantic` + `server/src/index.ts:958` `POST /api/projects/:id/search/index` async, `web/src/components/Sidebar.tsx:62` toggle + score bar + `VECTOR` badge, `server/src/fsx.ts:10` `resolveInProject` guard, `storage/ksagent.db` `embedding_chunks` `vec_chunks`, `OpenAI text-embedding-3-small` + `Ollama nomic-embed-text` + `local MiniLM` 384-d fallback, incremental `contentHash`, `write_file`/`edit_file`/`apply_patch`/`delete_file` `upsert/delete` (`server/src/agent.ts:2027`), WAL `busy_timeout 10000` + `saveLock` concurrent-safe, backward compat (missing vec ≠ crash) | ✅ grep/glob | ✅ grep + embeddings | 🔶 embeddings | ✅ | ✅ embeddings + grep | ✅ grep | ✅ | ✅ embeddings | ✅ embeddings | ✅ embeddings (KS **95** now **best** vector+BM25+grep hybrid > Cody 85) |
...
*   **C10 85→88→91 (offline lane, e2e-proven 2026-09-07):** `llm.ts:165` omits `Authorization` when `apiKey` empty + `SettingsModal.tsx:28` `PROVIDER_PRESETS` Ollama/LM Studio/vLLM localhost + empty-key provider create (`index.ts:2112`, `keyPreview:''`) + local embeddings default (`store.ts:1339`/`1270`/`1472`) + **new: `POST /api/settings/providers/check` local reachability probe (no key sent, 5s timeout, `isLocal` flag, `server/src/index.ts:2178`) + `POST /api/settings/providers/ollama-models` local discovery via `/api/tags` (`index.ts:2204`) + one-click clear-saved-key go-offline (`PATCH clearApiKey:true`, `index.ts:2159`, `SettingsModal.tsx:898` checkbox) + local LLM chat titles with empty key (`index.ts:1215` gate now `baseUrl && model`, auth still omitted).** LAN-stub proof (prior): all requests `auth=no`, SSE reply streamed+persisted, heuristic title, `embeddingCount:1`, hybrid hit `1.0`. Live-probe proof (this lane, `:8091`): create empty-key provider → `201 keyPreview:''`; `check {localhost:9}` → `reachable:false isLocal:true` in 11ms; invalid URL → `400`; `ollama-models {11434}` → `ok:false models:[]` in 18ms (no hang); set-then-clear key → `••••7890` → `''` with plain PATCH keeping key (backward compat). Now edges DeepSeek weights (90) on turnkey offline *agent* UX — weights portability itself still favors raw GGUF.
...
| | **TOTAL if C13 folded (/1300)** | **1222** | **982** | **778** | **739** | **969** | **781** | **846** |
| | **AVERAGE if /1300** | **94.0** | **75.5** | **59.8** | **56.8** | **74.5** | **60.1** | **65.1** |

> With C13 folded, KS is #1 (1222 vs Opencode 982, lead **+240**) and ties Opencode on delegation itself (**KS 88 = Opencode 88**, Claude 92 stays top). Math check: 1134+88=1222, 1222/13=94.0 (prior 1173 was an arithmetic error: 1131+68=1199, not 1173).
...
| 10 | Cody | 690 | 57.5 | Best for enterprise code search (KS now 95 hybrid `server/src/store.ts:1919`) |
| 11 | Claude Code | 686 | 57.2 | Best reasoning, worst lock-in + cost (KS now 96 parity `server/src/agent.ts:915`) |
...
| **F. Air-gapped / offline / local LLM** | **91** | 84 | 10 | **90** | 70 | 10 | 86 | **88** | 82 | 12 |
...
**How to read:** Highest in your row = best pick *for that job*. **After 5 lanes + vector (Lane 6) + offline lane:** D 78→92 vs 98 (gap closed via `web/src/components/FilesPane.tsx:604` `vscode-extension/src/extension.ts:45` `vscode-extension/package.json:18` `vsce package`), C 86→96 vs 98 (via `server/src/agent.ts:915` `server/src/store.ts:390`), **H 55→80→95 vs 85 (vector+hybrid `server/src/store.ts:390` `embedding_chunks` `FLOAT32[384/768]` per CHUNK + `server/src/store.ts:1919` `0.5*vector+0.3*BM25+0.2*grep` + `sqlite-vec/HNSW` + `OpenAI`/`Ollama`/`local` — benchmark `bench_vector` 200 files `user auth` `10/10` `0.733` `hybrid` vs `grep` `0`, `95` honest > Cody `85`)**, **E 92→98 parity via `KS_DOCKER_JAIL=1` `server/src/docker.ts:10` `server/src/index.ts:243`**, **F 88→91 outright #1 via `POST /api/settings/providers/check` + `ollama-models` + `clearApiKey` go-offline (`server/src/index.ts:2178`/`2204`/`2159`)** — KS now wins A/B/C/D/E/**F**/**H**/J, ties K at 88 (Claude 92 stays top on orchestration), competitive on I.
...
|---|---|---|---|---|---|
| **Self-Hoster** (privacy + mobile + offline) | C4×2, C5×1.5, C10×1.5, C3×1.5 | **KS Agent 94.5** | Opencode 75.0 | OpenHands 73.9 | **#1** |
| **IC Engineer** (reasoning + IDE + search + terminal) | C2×2, C6×2, C9×1.5, C7×1.5 | **KS Agent 94.3** | OpenHands 73.7 | Opencode 73.1 | **#1 (was #4 → #1 via C2 86→96 `server/src/agent.ts:915` `server/src/store.ts:390` + C6 78→92 `web/src/components/FilesPane.tsx:604` `vscode-extension/src/extension.ts:45` + C9 92→98 `server/src/docker.ts:10`)** |
...
| **Enterprise** (search + security + isolation + reasoning) | C9×2, C2×3.5, C12×1.5 | **KS Agent 94.8** | OpenHands 77.4 | Opencode 75.9 | **#1 (was #3 → #1 via C9 92→98 `server/src/docker.ts:10` `server/src/index.ts:243` + C2 86→96 + C12 90→96 `server/src/index.ts:4705` + H 55→95)** |

> **Honest conclusion — #1 in every persona after 6 lanes + offline lane (was #4 IC, #3 Enterprise):** KS is #1 *generalist* **1134/94.5 → 1148/95.7 with H 95** and #1 *self-hoster* (94.5) *IC Engineer* (94.3 → 95.1 with H 95) *startup builder* (94.3) *enterprise* (94.8 → 96.1 with H 95) — **IC 78.4→94.3→95.1** and **Enterprise 79.9→94.8→96.1** closed via Lane 1 C2/H 55→80, **Lane 6 H 80→95 vector+hybrid (beats Cody 85)** `server/src/store.ts:390` `embedding_chunks` `FLOAT32[384/768]` per CHUNK `server/src/store.ts:1919` `0.5*vector+0.3*BM25+0.2*grep` + `sqlite-vec/HNSW` `OpenAI`/`Ollama`/`local` + `bench_vector` `10/10` `0.733` `hybrid`, Lane 2 C6 78→92, Lane 3 C9/E 92→98, Lane 4 C11 88→94, Lane 5 C12 90→96, **offline lane C10 88→91 (`server/src/index.ts:2178`/`2204`/`2159`)**. Previous #4/#3 gaps were IDE polish (C6 78→92 done), hybrid search H 55→80 done Lane 1 `server/src/store.ts:1496`, Docker jail E 92→98 done Lane 3, onboarding presets done Lane 4 (wizard/auto-detect never existed — see evidence note), extensibility C12 90→96 done Lane 5, **vector H 80→95 done Lane 6 `server/src/store.ts:390` `server/src/store.ts:1919` `web/src/components/Sidebar.tsx:62` `95` > Cody `85`**, **parallel fan-out done (`delegate_task` + e2e, C13 88 = Opencode)**. Honestly left: real `git worktree add` per sub-agent (path reserved, `agent.ts:2783` pending — same caveat as Opencode `#34216`), no auto-commit-per-change (Aider-style), single-tenant default.
...
*   Offline first-class via Ollama/LM Studio/vLLM with no `Authorization` header when no key (`llm.ts:165`) — fully air-gapped after `npm run build` + `ollama pull` (e2e-proven: LAN stub, `auth=no`, local embeddings; live-probed `:8091`: `POST /api/settings/providers/check` + `ollama-models` + `PATCH clearApiKey` go-offline, `server/src/index.ts:2178`/`2204`/`2159`). No Ollama auto-detect wizard exists in-tree — discovery is an explicit `List Ollama models` button (`SettingsModal.tsx:912`), not background detection.
...
| **Air-gapped / offline** | **KS+Ollama 91 / DeepSeek 90** | Continue 88 / Aider 85 |
...
| **Worst for** | Cloud background fleets (no Devin-style multi-tenant worker cloud) | Phone / preview | Cheap/budget | Needs harness | Light edits, cost | Phone / server |
...
| **Parallelism** | **Sub-agents 88 = Opencode** — intra-chat `delegate_task` + N chats/projects concurrent | **Sub-agents 88** — General/Explore via `task`, worktree pending | **92** Plan/Explore | — | 80 Swarm | 40 | 
...
Yes — **vector+hybrid 95/100 (beats Cody 85)** (`server/src/store.ts:390` `embedding_chunks` FLOAT32[384/768] per chunk 400-600 tokens + `store.ts:1233` `chunkContentForEmbedding` + `store.ts:1270` `localEmbed` + `store.ts:1458` `embedMany` batch 64 + `store.ts:1919` `semanticSearch` 0.5*vector+0.3*BM25+0.2*grep + `store.ts:1496` sqlite-vec `vec0` + HNSW fallback + `server/src/agent.ts:658` `AGENT_TOOLS` + `server/src/index.ts:890` semantic API + `web/src/components/Sidebar.tsx:62` toggle). Lightweight pure-JS, no heavy deps, proven on 200-file bench `10/10 hybrid 0.733 vs grep 0`.

**Does KS Agent have sub-agents / parallel agents like Opencode?**
Yes — native intra-chat `delegate_task` (`research`/`explore`/`fix`/`write`/`general`, `server/src/agent.ts:1033` def, `:2733` exec, `store.ts:449`/`758`, `index.ts:835` routes) with forced fan-out per mode (`agent.ts:3117`) — e2e 2026-09-07: swarm forced 2 delegates, both `done` with messages (K 88 = Opencode 88, Claude 92 tops orchestration). Plus multi-chat parallelism: each chat is an independent agent (`server/src/index.ts:1075` `generations` Map per chatId, `server/src/index.ts:1485` 409 guard per chat, `server/src/index.ts:1375` `/api/generations`), so open 3 chats → 3 agents run concurrently, each with own plan/preview/PTY (`index.ts:219`). Caveat shared with Opencode: real `git worktree add` per sub-agent still pending (`agent.ts:2783`).
...
Yes — winning stack: **KS Agent (self-host, phone, native fan-out, preview)** for orchestration + **Opencode/Claude sub-agents** via MCP (`server/src/mcp.ts:325` `connectMCPServer`, 4 transports) or just git worktrees when you want a second opinion. Share `project/` on disk.
...
*   **Evidence (2026-09-07, re-audited — every cite checked against the tree):** KS Agent details verified by reading this repo's code: `server/src/agent.ts:15` large-edit prompt, `agent.ts:198` Skill guard, `agent.ts:1168` jail + `agent.ts:2959` history truncation 90k + `agent.ts:2673` `ask_question` blocking + `agent.ts:1033`/`2733` `delegate_task` (e2e: swarm 2 delegates `done`), `server/src/fsx.ts:10` realpath, `server/src/llm.ts:165` no-key + `server/src/llm.ts:125` streaming, `server/src/index.ts:211` PTY + `1075` `generations` Map per chatId + `1485` 409 guard + `1375` `/api/generations` + `2964` IDE routes + `5431` dual-stack `isPortReachable`/`5453` `fetchPreviewWithFallback`/`5588` preview proxy + `1546` full-mode 25×1200×15k + `1614` preserveActivities + `server/src/store.ts:377` WAL `busy_timeout 10000` + `390` `embedding_chunks` + `1233` `chunkContent` + `1270` `localEmbed` + `1919` `semanticSearch` + `253` `semantic_search` type + `505` `chmod 600`, `web/src/components/FilesPane.tsx:596` ghost + `⌘K`, `web/src/components/App.tsx:39` context mode, `vscode-extension/package.json:18` VS Code ext, `server/src/mcp.ts:325` MCP 4 transports + `server/src/lsp.ts:365` LSP 6. Dropped as non-existent: `OnboardingWizard.tsx`, `QUICK_PRESETS`, `needsKey`, `EXT_INSTALL_CMD`, keychain hints, Quick Setup wizard, Ollama auto-detect, `maxParallel`/`previewGate`, Preview-Team watch, `server.host:'0.0.0.0'` in project vite configs. Opencode sub-agent evidence: `opencode.ai/docs/agents` Build/Plan/General(`subagent`)/Explore + `#34216` worktree pending.
*   **Honest vs prior inflated version:** Inflated 1135 (94.6, claimed beats all personas) reverted to honest **1131 (94.3)** with measured lifts (+91 over original 1040, not +95) and honest scenario/persona rankings — plus C13 folded separately (KS 1219/93.8 with delegation at 88). **Offline lane (2026-09-07) moves the honest baseline to 1134/94.5 (+94) via C10 88→91 only — numerically near the old inflated 1135 but through a different, live-probed path (`server/src/index.ts:2178`/`2204`/`2159`, `:8091` curl evidence), not restored inflation.** See §4.2 evidence notes + §3 row (✅).
