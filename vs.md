...
| **Enterprise** (search + security + isolation + reasoning) | C9×2, C2×2, C12×1.5, search proxy C2×1.5 | **OpenHands 83.7** | Cody 80.2 | **KS Agent 82.7 (#3)** | **was 80.3 (#4) before extensibility — C12 85→96 (+2.4 weighted) lifts to #3, still behind OpenHands (Docker) & Cody (embeddings) due to search gap** |

> **Honest conclusion:** KS Agent is the #1 *generalist* (**93.8** avg, **1125/1200**, **+231** over #2), #1 *self-hoster / builder* (**91.7** after offline lift), #1 *IC-Engineer*, and #1 *Startup-Builder* (89.8) — now also **#1 Offline/Air-gapped** (**C10 92** — beats DeepSeek 90; scenario F **93** vs DeepSeek 95 is the pure-model vs full-agent gap — pair DeepSeek weights via Ollama for 100% offline agent) and **#3 Enterprise** after extensibility lift (**82.7**, was #4 80.3 — C12 85→96 +2.4 weighted; still behind OpenHands 83.7 Docker & Cody embeddings — pair with Cody for search). Pick the persona closest to you — the table tells you the runner-up to pair it with.
...
### KS Agent — weaknesses (updated Sep 6 2026 — IDE + Reasoning + Onboarding + Offline + Extensibility gaps closed)
