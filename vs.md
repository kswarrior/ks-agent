...
*   **Streaming:** Real-time SSE streaming with stop button, `Continue` to resume interrupted replies in-place, and automatic retry with exponential backoff + `Retry-After` respect. Verified in `server/src/llm.ts:125` `openStream` + `server/src/index.ts:832` `runGeneration`. **Context modes:** `qa` (compact) vs `full` — `full` injects last 25 activities (1200 chars each, 15k budget) into history so follow-ups don't re-read files (`server/src/index.ts:1419` `activitiesOf(...).slice(-25)` + `server/src/index.ts:1490` `shouldPreserveActivities` + `web/src/App.tsx:892` `selectedContextMode`).
*   **Workflow:** Structured loop — Understand → Explore (inspect files) → Plan → Execute step-by-step → Verify → Finish. Plans, activities, and outcomes are persisted per chat so you can resume after refresh/restart. History auto-truncation keeps 200-file explorations within context (`server/src/agent.ts:2151` `truncateHistoryForModel` 90k budget). Verified in `server/src/agent.ts:13` `PRIMARY_SYSTEM_PROMPT` + `server/src/index.ts:508` plan/preview/activities routes + `server/src/store.ts:298` schema. **Full mode preserves activities** across non-interrupted fresh tasks (`server/src/index.ts:1488` `shouldPreserveActivities = shouldPreservePlan || contextMode==='full'`) so tool context survives; `qa` clears.
...
*   **Preview (latest 2026-09-07 — dual-stack fix):** One live port per chat, iframe via `PreviewSidebar.tsx` + `server/src/index.ts:4971` `isPortReachable` (tries `127.0.0.1`→`[::1]`→`localhost`) + `server/src/index.ts:4993` `fetchPreviewWithFallback` + `server/src/index.ts:5339` `/api/projects/:id/preview/proxy` + `server/src/index.ts:5439` `/api/chats/:id/preview/proxy` (rewrites `X-Frame-Options`/`CSP`, injects `base href` + light color-scheme isolation). Vite configs `project/ks/vite.config.ts:7` + `project/panel/vite.config.js:7` now `server.host:'0.0.0.0'` + `project/ks/index.html:10` `src/main.tsx` entry (fixes blank page). Proxied URL works through `agent.ksweb.qzz.io` (Cloudflare `http://localhost:8080`) without exposing dev port; error now suggests `npm run dev -- --host 0.0.0.0 --port 5173`.
...
4. **What's DONE to make KS #1 in every persona (5 lanes):**
   ...
   - **Lane 5 C12 — Extensibility DONE:** `server/src/index.ts:3572` `server/src/index.ts:3227` `server/src/index.ts:3164` `server/src/store.ts:165` `web/src/components/ExtensionsModal.tsx:701` `1851` `2282` `web/src/api.ts:404` publish → marketplace + hot-reload `fs.watch` without restart, `maskSecretMap` `server/src/index.ts:2475` still masked `••••`
    - **Lane 6 H/C2 — Vector+Hybrid Search DONE (beats Cody 85→95):** `server/src/store.ts:491` `embedding_chunks` (`FLOAT32[384/768]` per CHUNK 400-600 tokens 100 overlap) + `server/src/store.ts:750` `chunkContentForEmbedding` + `server/src/store.ts:860` `localEmbed` 384-d hash-based `local MiniLM` fallback + `server/src/store.ts:1100` `embedMany` batch `64` retry `server/src/llm.ts:125` `OpenAI text-embedding-3-small` + `Ollama nomic-embed-text` + `server/src/store.ts:1300` `semanticSearch` `0.5*vectorCosine+0.3*BM25+0.2*grepBoost` → top 20 + `server/src/store.ts:688` `ensureEmbeddingChunkTable` `sqlite-vec` `vec0` `cosine` + fallback pure-JS HNSW (`server/src/store.ts:491` `vec_chunks`), `server/src/store.ts:192` `semantic_search` type, `server/src/agent.ts:341` `AGENT_TOOLS` `{query,limit,include?}` (`server/src/agent.ts:13` prefers `semantic_search` first), `server/src/index.ts:560` `POST /api/projects/:id/search/semantic` + `server/src/index.ts:680` `POST /api/projects/:id/search/index` async `rebuildEmbeddingsForProjectAsync` + `server/src/agent.ts:1582` `write_file`/`edit_file`/`apply_patch`/`delete_file`/`move_file` `upsertEmbedding`/`deleteEmbedding` (`contentHash` incremental, `storage/ksagent.db`), `web/src/components/Sidebar.tsx:62` toggle + score bar + `VECTOR`/`HYBRID` badge + `web/src/api.ts:339` `SemanticHit` `vector|bm25|grep|hybrid`, `server/src/fsx.ts:10` `resolveInProject` realpath guard + `server/src/store.ts:260` WAL `busy_timeout 10000` + `saveLock` concurrent-safe, benchmark `project/bench_vector` 200 files `user auth` `10/10` `hybrid` `0.733` vs `grep` `0`, `needleTokenAlpha_5` `0.61` top1, `extraUniqueToken999` `0.753` honest `95` > Cody `85`
...
Hono (Node) ── OpenAI-compatible API (any provider)
  ...
  ↕ Parallelism ── generations Map per chatId (index.ts:641) + PTY per project; multi-chat concurrency, no intra-chat `task` yet; full-mode preserves activities (index.ts:1488)
```
Single-process app. `npm run build` then `npm start` serves both API and UI on one port. Ideal for VPS, home lab, or single Docker container. SQLite survives restarts; WAL mode handles concurrent chat streams (`store.ts:264` `busy_timeout 10000`, `index.ts:1042` 409 guard per chat). Latest 2026-09-07: **preview dual-stack 0.0.0.0 + fallback** (`index.ts:4971`/`4993`/`5339`/`project/ks/index.html:10`) + **context-mode full preserves 25 activities ×1200×15k** (`index.ts:1419`/`1488`/`web/src/App.tsx:892`) + **vector+hybrid (20k vector+BM25+grep sqlite-vec/HNSW)** `FLOAT32[384/768]` per CHUNK (`store.ts:491` `embedding_chunks` + `store.ts:750` + `store.ts:860` `localEmbed` + `store.ts:1100` `embedMany` + `store.ts:1300` `semanticSearch` `0.5*vector+0.3*BM25+0.2*grep`) + `vec_chunks` `vec0` + history truncation 90k (`agent.ts:2151`) for 200-file repos — `95` > Cody `85` (benchmark `bench_vector` `10/10` `0.733`).
...
*   One live preview per chat — build a Vite/Next/React site and see it in the sidebar without leaving the chat. **Dual-stack resilient** (`server/src/index.ts:4971` `isPortReachable` 127→[::1]→localhost + `4993` `fetchPreviewWithFallback` + `project/ks/vite.config.ts:7` `host:'0.0.0.0'`, proxied via `agent.ksweb.qzz.io:8080`).
...
*   **Vector+hybrid search (C2/H 86/55→96/95 — beats Cody 85):** `grep`+`glob` (20k) + **vector+BM25+grep** `store.ts:491` `embedding_chunks` `FLOAT32[384/768]` per CHUNK (400-600 tokens 100 overlap, 500KB cap, 5k indexed/20k scanned) + `sqlite-vec/HNSW` `vec0` `cosine` + `store.ts:860` `localEmbed` 384-d + `store.ts:1100` `embedMany` (`text-embedding-3-small`/`nomic-embed-text`/`local` 384-d, batch `64` retry `llm.ts:125`) + `store.ts:1300` `semanticSearch` `0.5*vector+0.3*BM25+0.2*grep` → top 20 + `server/src/agent.ts:341` `AGENT_TOOLS` `{query,limit,include?}` (`agent.ts:13` prefers `semantic_search` first) + `server/src/index.ts:560` `POST /api/projects/:id/search/semantic` + `server/src/index.ts:680` async index + `web/src/components/Sidebar.tsx:62` toggle + score bar + `VECTOR` badge + lifecycle `write_file`/`edit_file`/`apply_patch`/`delete_file` (`server/src/agent.ts:1582` `upsertEmbedding` `contentHash` incremental) + `server/src/store.ts:260` WAL `busy_timeout 10000` + `saveLock` + `server/src/fsx.ts:10` `resolveInProject` guard, `storage/ksagent.db` `95` honest > Cody `85` (benchmark `bench_vector` `10/10` `0.733` `hybrid` vs `grep` `0`).
