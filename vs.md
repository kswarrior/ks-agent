...
*   Offline first-class via Ollama/LM Studio/vLLM with no `Authorization` header when no key (`llm.ts:165`) — fully air-gapped after `npm run build` + `ollama pull` (e2e-proven: LAN stub, `auth=no`, local embeddings; live-probed `:8091`: `POST /api/settings/providers/check` + `ollama-models` + `PATCH clearApiKey` go-offline, `server/src/index.ts:2177`/`2203`/`2156`). No Ollama auto-detect wizard exists in-tree — discovery is an explicit `List Ollama models` button (`SettingsModal.tsx:910`), not background detection.
