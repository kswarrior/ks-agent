...
function SubAgentChat({ subAgentId, streaming }: { subAgentId: string; streaming: boolean }) {
  ...
  // live poll while streaming or while sub-agent is working (status will be polled by parent, but we poll messages every 1.5s)
  useEffect(() => {
    if (!streaming) return
    const id = setInterval(() => {
      api.listSubAgentMessages(subAgentId).then((list) => setMsgs(list)).catch(() => {})
    }, 1500)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8 }}>
      {msgs.map((m) => (
        <div key={m.id} className={m.role === 'user' ? 'msg-user' : m.role === 'assistant' ? 'msg-assistant' : 'msg-assistant'} style={m.role === 'tool' ? { background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 10px' } : undefined}>
